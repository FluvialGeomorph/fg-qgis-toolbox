...
if ($trial.schema_version -ne 1 -or $trial.status -ne 'prepared-not-analyst-qualified' -or
...
Assert-TrialHash (Join-Path $trial.r_library 'fgqgis/rscripts/fg_review_stream_network.rsx') $trial.script_sha256
...
try {
    ...
    Write-Output "Opened the isolated trial. Input: $($trial.input)"
